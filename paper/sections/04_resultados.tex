...
Esta seção apresenta a avaliação do serviço de metadados proposto. A avaliação é organizada em seis partes: a metodologia experimental adotada, os testes funcionais que verificam a corretude do provisionamento, os testes de desempenho que quantificam a latência e a escalabilidade do serviço, a avaliação de alta disponibilidade baseada em consenso RAFT, uma comparação de funcionalidades com serviços de metadados de provedores de nuvem públicos e a discussão dos resultados e de suas limitações.
...
A coleta de latência foi realizada com a ferramenta \texttt{hey}, disparando 5000 requisições por configuração com concorrência de 50 requisições simultâneas a partir de um contêiner gerador de carga. Essa carga HTTP é idêntica em todas as configurações; o que varia entre elas é o número de contêineres em execução no host, que determina o volume de escritas realizado a cada ciclo de sincronização e o consumo de recursos do host. O banco SQLite foi aberto em modo WAL (\textit{Write-Ahead Logging}), com \textit{busy timeout} de 5~s e uma única conexão aberta (\texttt{SetMaxOpenConns(1)}), o que serializa todo o acesso ao banco dentro do processo do serviço. Os resultados de latência são reportados nos percentis p50, p95 e p99, seguindo a prática adotada em sistemas de produção para caracterizar tanto o comportamento típico quanto os casos de cauda da distribuição.
...
\begin{enumerate}
    ...
    \item \textbf{Entrega de configuração de rede}: a configuração entregue pelo \textit{endpoint} \texttt{/network-config} foi servida como YAML válido, com as interfaces reais reportadas pelo Incus, e capturada para comparação com a saída de \texttt{ip addr} na instância. A aplicação dessa configuração pelo \textit{cloud-init} não foi exercitada, pois a imagem-base utilizada desabilita a gestão de rede do \textit{cloud-init} (Seção~\ref{subsec_metodologia}).
...
Entrega de \textit{network-config} & Aprovado (conteúdo servido) \\
Propagação de estado após reinício & Aprovado \\
\hline
\end{tabular}
\end{table}

Todas as propriedades verificadas foram atendidas, com a ressalva de que a entrega de \textit{network-config} foi validada apenas quanto ao conteúdo servido, não quanto à sua aplicação. Em particular, uma inicialização real de \textit{cloud-init} selecionou o \textit{datasource} \texttt{NoCloud} de rede, obteve seus metadados e \textit{user-data} do serviço (respostas HTTP 200) e concluiu com \texttt{cloud-init status: done}, evidenciando a compatibilidade funcional de ponta a ponta.
...
A latência de sincronização (intervalo entre a criação de uma instância no Incus e a disponibilização de seu registro de metadados) foi medida, em uma única execução, em aproximadamente \textbf{20{,}5~s}. Esse valor é consistente com a janela do ciclo de sincronização periódica adotado (\textit{polling} a cada 10~s) somada ao tempo de inicialização da instância e de obtenção de endereço de rede.

\subsubsection{Escalabilidade}

Para avaliar o comportamento com o aumento do número de instâncias simultâneas, mediu-se a latência de resposta do \textit{endpoint} \texttt{/meta-data} para configurações de 5 a 200 contêineres. A hipótese inicial era de que, por utilizar SQLite como mecanismo de persistência, o serviço apresentasse degradação de desempenho sob alta concorrência de escrita, dado que o SQLite serializa operações de escrita \citep{sqlite2024}. A Tabela~\ref{tab:escalabilidade} apresenta os resultados.

\begin{table}[ht]
\centering
\caption{Escalabilidade: latência de \texttt{/meta-data} e memória utilizada no host por número de contêineres em execução.}
\label{tab:escalabilidade}
\begin{tabular}{lccccc}
\hline
\textbf{Instâncias} & \textbf{p50 (ms)} & \textbf{p95 (ms)} & \textbf{p99 (ms)} & \textbf{Erros} & \textbf{Mem. host (MB)} \\
\hline
...
Os dados \textbf{não corroboram} a hipótese de degradação por serialização de escrita nessa faixa de escala: a latência permaneceu estável (p50 entre 12 e 16~ms; p99 entre 77 e 93~ms) e a taxa de erro foi nula em todas as configurações, até 200 instâncias sobre 4 vCPUs. Esse resultado decorre da natureza do fluxo predominante, majoritariamente requisições de leitura (\texttt{GET}) de metadados, enquanto as escritas de sincronização ocorrem apenas a cada 10~s e são absorvidas pelo modo WAL e pelo \textit{busy timeout} configurados no SQLite (Seção~\ref{subsec_metodologia}). Conclui-se que a serialização de escrita não é um gargalo no perfil de carga avaliado; uma eventual degradação exigiria contagens de instâncias ou taxas de escrita substancialmente superiores, o que se registra como limite medido e não como tendência de degradação linear.
...
Reeleição após falha do líder & 2{,}40~s (uma execução) \\
...
Para contextualizar as capacidades do serviço proposto, a Tabela~\ref{tab:comparacao_campos} compara os campos de metadados disponibilizados com os oferecidos pelo AWS Instance Metadata Service \citep{aws_imds}, pelo serviço de metadados do Google Cloud Platform \citep{gcp_metadata} e pelo serviço de metadados do OpenStack \citep{openstack_metadata}.
...
\begin{itemize}
    ...
    \item \textbf{Cobertura parcial de campos de metadados}: o serviço implementa o subconjunto de campos necessários aos módulos mais comuns do \textit{cloud-init}, mas não cobre todos os campos de provedores como AWS e GCP \citep{aws_imds, gcp_metadata}. Cargas que dependam de campos específicos podem requerer extensões.
