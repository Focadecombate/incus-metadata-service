...
\begin{frame}{Problema}
    \begin{itemize}
        \item O \textbf{Incus}~\cite{incus2024} (\textit{fork} comunitário do LXD~\cite{incus_announcement}) não
              oferece um serviço de metadados acessível via rede (TCP/IP).
        \item A entrega de configuração existente (\textit{datasource} LXD via
              ...
              estágios do cloud-init em VMs~\cite{incus_cloudinit_forum}.
        \item Consequências:
        \begin{itemize}
...
\begin{frame}{Comparação entre as Abordagens}
    \begin{table}
        ...
        \begin{tabular}{lcccccc}
            \toprule
            \textbf{Solução} & \textbf{Autônomo} & \textbf{Endpoint HTTP cloud-init} &
...
\begin{frame}{Visão Geral}
    \begin{itemize}
        ...
        \item A instância é identificada pelo \textbf{endereço IP de origem} da
              requisição: sem agentes na instância; a única configuração é o
              \texttt{seedfrom} do NoCloud, definido uma vez na imagem-base.
...
\begin{frame}{Sincronização de Instâncias}
    ...
    \begin{enumerate}
        ...
        \item Para cada instância, um evento \texttt{instance.sync} é publicado, com
...
\begin{frame}{Endpoints da API}
    ...
    \end{itemize}
    \vspace{0.3cm}
    Endpoints internos (\texttt{/internal/vendor/*}): criação, consulta e atualização
    de vendor-data por fornecedor; não expostos às instâncias.
\end{frame}

\begin{frame}{Descoberta do Serviço pelas Instâncias}
    \begin{itemize}
        \item \textit{Datasource} NoCloud em modo rede: \texttt{seedfrom} aponta para
              \texttt{/configs/} do serviço; o cloud-init concatena
              \texttt{meta-data}, \texttt{user-data}, \texttt{vendor-data} e
              \texttt{network-config}.
        \item O \texttt{seedfrom} precisa existir antes de qualquer consulta: fica em
              um \textit{drop-in} do cloud-init gravado na \textbf{imagem-base}
              (uma configuração por imagem, não por instância; sem agentes).
        \item Endereço do serviço, duas formas:
        \begin{itemize}
            \item \textit{link-local} \texttt{169.254.169.254} na \textit{bridge}
                  \texttt{incusbr0} + NAT da porta 80, para hosts dedicados;
            \item \textit{gateway} da \textit{bridge} + porta do serviço, quando o
                  host é uma VM de nuvem pública (endereço já em uso pelo provedor).
                  Forma usada nos experimentos.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Alta Disponibilidade via RAFT}
    ...
    \begin{itemize}
        ...
        \item \textbf{Leituras em qualquer nó}: todos mantêm cópia local do estado
              (leituras eventualmente consistentes), distribuindo a carga de consultas.
...
\begin{frame}{Metodologia de Avaliação}
    \begin{itemize}
        \item Ambiente: GCE \texttt{e2-standard-4} (4 vCPUs, 15~GiB), Ubuntu 24.04
              ...
              Alta disponibilidade: 3 VMs \texttt{e2-standard-2}, uma réplica em cada.
        \item De 5 a 200 contêineres em execução, criados de uma imagem-base com o
              \texttt{seedfrom}; carga HTTP fixa com \texttt{hey} (5000 requisições,
              concorrência 50), percentis p50/p95/p99.
        \item SQLite em modo WAL, \textit{busy timeout} de 5~s e uma única conexão
              (\texttt{SetMaxOpenConns(1)}).
        ...
        \item A imagem-base desabilita a rede do cloud-init (\texttt{eth0} por DHCP
              via Netplan): \texttt{network-config} foi servido, não aplicado.
    \end{itemize}
\end{frame}
...
\begin{frame}{Desempenho: Latência da API}
    ...
    \begin{itemize}
        ...
        \item Latência de sincronização (criação no Incus $\rightarrow$ metadados
              disponíveis): $\approx$~20{,}5~s em uma execução (ciclo de 10~s + \textit{boot} e DHCP).
    \end{itemize}
\end{frame}
...
\begin{frame}{Desempenho: Escalabilidade}
    ...
    \begin{itemize}
        ...
        \item \textbf{Não corroborada} até 200 instâncias: latência estável e zero
              erros; carga dominada por leituras, e escritas a cada 10~s absorvidas
              pelo modo WAL e pelo \textit{busy timeout}.
    \end{itemize}
\end{frame}
...
\begin{frame}{Alta Disponibilidade (\textit{cluster} de 3 nós)}
    \begin{table}
        ...
        \begin{tabular}{ll}
            ...
            Reeleição após falha do líder & \textbf{2{,}40~s} (uma execução) \\
...
\begin{frame}{Discussão e Limitações}
    \begin{itemize}
        \item Cobre os campos essenciais dos módulos mais comuns do cloud-init
              (\texttt{set\_hostname}, \texttt{users\_groups}, \texttt{write\_files},
              \texttt{runcmd}, rede).
        ...
        \item \textbf{Ameaças à validade}: host único nos testes de desempenho;
              ...
              \textit{polling}; \texttt{network-config} servido mas não aplicado;
              sincronização e reeleição medidas em execução única.
    \end{itemize}
\end{frame}
...
\begin{frame}{Conclusão}
    \begin{itemize}
        ...
        \item Identificação por IP de origem: sem agentes nas instâncias; a única
              configuração é o \texttt{seedfrom} na imagem-base.
...
\begin{frame}{Trabalhos Futuros}
    \begin{enumerate}
        \item Eliminar a imagem-base pré-configurada: \textit{datasource} nativo do
              Incus no cloud-init ou \texttt{seedfrom} via linha de comando do
              \textit{kernel} (VMs).
        ...
        \item \textit{Benchmarks} em maior escala, com repetições para análise estatística.
        \item Validar a aplicação do \texttt{network-config} com a rede do cloud-init
              habilitada.
    \end{enumerate}
\end{frame}
